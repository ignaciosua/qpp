#!/usr/bin/env python3
"""QPP that WORKS: compress all layers, then FT anchors, then generate."""
import gc, time, sys, json, math
...
print(f"BF16: {ppl_base:.4f} PPL | {orig_mb:.0f} MB",flush=True)

# ═══════ COMPRESS ALL (proven per-layer) ═══════
...
print(f"Compressing {len(an)} attn + {len(mn)} mlp...",flush=True)

all_names = mn + an  # MLP first
ok=0; db=0; qb=0
...
for i,n in enumerate(all_names):
    ...
    if i%30==0: print(f"  [{i+1}/{len(all_names)}] ok={ok}",flush=True)
gc.collect(); torch.cuda.empty_cache()
ppl_qpp=p(); comp_mb=persistent_model_bytes(m)/1e6
print(f"Compressed: ppl={ppl_qpp:.2f} comp={orig_mb/comp_mb:.1f}x",flush=True)

# ═══════ QPP-AWARE FT (recover quality) ═══════
...
print(f"\n{'='*55}")
print(f"QPP + FT — RESULT")
print(f"{'='*55}")
print(f"BF16:   {ppl_base:.4f} | {orig_mb:.0f} MB")
print(f"QPP:    {ppl_qpp:.2f} | {comp_mb:.0f} MB")
print(f"QPP+FT: {ppl_ft:.4f} | {final_mb:.0f} MB")
print(f"SAVED:  {savings:.1f}% ({orig_mb/final_mb:.1f}x)")
print(f"dPPL:   {ppl_ft-ppl_base:+.4f}")
...
rpt={"model":"Qwen2.5-0.5B","bf16_ppl":ppl_base,"qpp_ppl":ppl_qpp,"ft_ppl":ppl_ft,"dppl":ppl_ft-ppl_base,"orig_mb":orig_mb,"final_mb":final_mb,"savings_pct":savings}
outdir=Path(__file__).resolve().parent.parent/"outputs"/"qpp_works"
